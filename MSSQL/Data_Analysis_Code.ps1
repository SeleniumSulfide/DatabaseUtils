...
Param(
	...
	[string]$trgFQ, #Name of hte Target Database
...
if ($ScriptPath -eq "")
{
	Add-Content "c:\users\itgentd\desktop\ScriptPathBlank-Code.txt"  "blank"
	exit
}
...
elseif ([int]$srcCount[0] -ne [int]$trgCount[0]) #Compares the record counts and logs the error if necessary
{
	...
	Add-Content $file "`t`t$($srcFQ.ToUpper()) Record Count: $($srcCount[0])"
	Add-Content $file "`t`t$($trgFQ.ToUpper()) Record Count: $($trgCount[0])"
...
else
{
	...
	$sql = ""
	$sql = "$($sql)select *`r`n"
	$sql = "$($sql)from $table`r`n"
	$sql = "$($sql)tablesample ($minRecords rows)"
	
	Set-Progress $commFile "Analyzing: $table" "Getting $($srcFQ.ToUpper()) record set" $progressID 0
	...
	$sql = "Select count(*) from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = '$($tabInfo[0])' and TABLE_NAME = '$($tabInfo[1])'"
	...
	foreach ($srcRow in $srcRows)
	{
		...
		else
		{
			...
			for($x = 0;$x -lt $colCount[0];$x++)
			{
				...
				$dtSql = "select DATA_TYPE from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = '$($tabInfo[0])' and TABLE_NAME = '$($tabInfo[1])' and ORDINAL_POSITION = $($x+1)"
